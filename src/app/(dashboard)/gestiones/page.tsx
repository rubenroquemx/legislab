...
import { MEXICO_TIMEZONE } from '@/lib/date-utils';
import { 
  Plus, 
  Search, 
  LayoutList, 
...
export default function GestionesPage() {
      ...
      if (res.success && res.data && res.data.length > 0) {
        const mapped: GestionCiudadana[] = res.data.map((d: any) => {
          ...
          return {
            ...
            estatus: normalizeEstadoGestion(d.estatus),
  ...
  const handleCambiarEstado = async (gestionId: string, nuevoEstado: EstadoGestion) => {
    setGestiones(prev => prev.map(g => g.id === gestionId ? { ...g, estatus: nuevoEstado } : g));
    try {
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between pt-1 text-[11px]">
                          <span className="inline-flex items-center gap-1 text-blue-600 font-semibold group-hover:underline">
                            ...
                            <span>Ver Expediente / Oficio ➔</span>
                          </span>
